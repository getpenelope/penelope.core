...
(function ($) {
    ...
    function resultCellClicked(ev) {
        if (opts.onResultCellClicked) {
            ...
            opts.onResultCellClicked(dataObj, ev);
        }
    }
    ...
    function appendChildRows(treeNode, belowThisRow, adapter) {
        ...
        for (i = 0; i < treeNode.children.length; i += 1) {
            ...
            for (col1 = 0; col1 < pivotCols.length; col1 += 1) {
                ...
                sb.clear();
                sb.append('<td>');
                sb.append(opts.formatNormalFunc(result));
                sb.append('</td>');
    ...
    function makeCollapsed(adapter, $obj) {
        var i, i1, col, result, $pivottable,
            sb = new lib.StringBuilder('<table class="pivot table">'),
            gbCols = adapter.alGroupByCols,
            pivotCols = adapter.uniquePivotValues,
            rowSum = 0.0;
    ...
    function AdapterObject() {
        this.alGroupByCols = [];
        this.pivotCol1 = null;
        this.resultCol1 = null;
        this.pivotCol2 = null;
        this.resultCol2 = null;
        this.bInvSort = false;
        this.tree = { children: [] };
    ...
    AdapterObject.prototype.parseJSONsource = function (data) {
        ...
        for (cellIndex = 0, cellcount = data.columns.length; cellIndex < cellcount; cellIndex += 1) {
            ...
            else if (col.colvalue === 'worked_days_label') {
                this.pivotCol1 = cell;
            }
            else if (col.colvalue === 'estimated_days_label') {
                this.pivotCol2 = cell;
            }
            else if (col.colvalue === 'worked_days') {
                this.resultCol1 = cell;
            }
            else if (col.colvalue === 'estimated_days') {
                this.resultCol2 = cell;
            }
        }
        ...
        for (rowIndex = 0, rowcount = data.rows.length; rowIndex < rowcount; rowIndex += 1) {
            ...
            //pivot 1
            pivotValue = trim(cells[this.pivotCol1.colvalue]);
            pivotSortBy = trim(cells[this.pivotCol1.sortbycol]);
            result = trim(cells[this.resultCol1.colvalue]);
            if (!curNode.pivotvalues) {
                curNode.pivotvalues = [];
            }
            newPivotValue = { pivotValue: pivotValue, result: result, sortby: pivotSortBy, dataid: this.pivotCol1.dataid };
            ...
            //pivot 2
            pivotValue = trim(cells[this.pivotCol2.colvalue]);
            pivotSortBy = trim(cells[this.pivotCol2.sortbycol]);
            result = trim(cells[this.resultCol2.colvalue]);
            newPivotValue = { pivotValue: pivotValue, result: result, sortby: pivotSortBy, dataid: this.pivotCol2.dataid };
            curNode.pivotvalues.push(newPivotValue);
            if (!lib.exists(this.uniquePivotValues, findPivotFunc, pivotValue)) {
                this.uniquePivotValues.push(newPivotValue);
            }
        ...
        this.uniquePivotValues.sort(this.getcomparer(this.bInvSort)[this.pivotCol1.datatype]);
    };
